        ...
        static Task<bool> GatherData(CancellationToken token)
        {
            return new Task<bool>(() =>
            {
                try
                {
                    ...
                    {
                        //.GetCustomAttributes(typeof(CoreStruct), false)
                        foreach (Method method in type.GetCustomAttributes(typeof(Method), false))
                        {
                            Add(method);
                        }
                        foreach (Property property in type.GetCustomAttributes(typeof(Property), false).Select(p => (Property)p))
                        {
                            Add(property);
                        }
                    }
